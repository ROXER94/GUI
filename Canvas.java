...
MouseMotionListener {
    ...
    final String defImgFileName = "Letters.bmp"; /** Default image file name */
    ...
    public void setNewImage(String imgFileName) throws IOException {
        ...
        // image to fit better in the canvas.
          ...
          double minX = Double.POSITIVE_INFINITY; double minY = minX; double
          maxX = Double.NEGATIVE_INFINITY; double maxY = maxX; for (Shape s:
          shapes) { BoundingBox box = s.tree.getBox(); if (box.lower.x < minX) minX =
          box.lower.x; if (box.lower.y < minY) minY = box.lower.y; if
          (box.upper.x > maxX) maxX = box.upper.x; if (box.upper.y > maxY) maxY
          = box.upper.y; } System.out.println("Area Covered: (" + minX + "," +
          minY + ") -- (" + maxX + "," + maxY + ")"); imgWidth = maxX;
          imgHeight = maxY;
    ...
    /** Process mouse event e.
     * @param e  MouseEvent
     * @return The location of the mouse when the event occurred in image
     *         coordinates.
     */
